...
class BinaryTree
{
	...
	/*
	 * Vertical Reflecting the tree with begining from the subroot
	 */  
	void mirror(Node* subRoot);


	/*
	 Check if the tree is sorted in order from left to right	 
	*/  
	bool isOrdered(Node* subRoot) const;

	/*
	 Print all the path of the tree start from subroot
	 */
	void printPaths(Node* subRoot, vector<T> t) const;

	int sumDistances(Node* subRoot, int sum) const;
};
